...
if(isset($_POST['register'])){
  ...
  if ($stmt->get_result()->fetch_assoc()['user'] > 0){
      header("Location: register_page.php?register_msg=Username is exist! Please enter new one");
      exit();
  }
  ...
  $query = "INSERT INTO Student (studentName, passwd) VALUES ('$username', '$hashed_password')"; 
  $result = mysqli_query($conn, $query);
  if(!$result){
      header('location: register_page.php?register_msg=You register failure');
      die("Query Failed". mysqli_error($conn));
  }else{
      header('location: main_page.php?register_msg=You register successfully');
  }
}
?>
